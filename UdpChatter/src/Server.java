...
public class Server {
	...
	private HashMap<String,ArrayList<String[]>> offLine = new HashMap<>();
	...
	public void saveMessage(String[] info,InetAddress senderIP,int senderPort) {
		String receiver = info[0]; String time = info[1]; String sender = info[2]; String msg = info[3];
		ArrayList<String[]> list = offLine.getOrDefault(receiver,new ArrayList<String[]>());
		list.add(new String[]{time,sender,msg});
			...
			try {
				...
				if(whatToDo.equals("reg")) {
					...
					// already registered
						...
						if(offLine.containsKey(userName) && offLine.get(userName).size() > 0) {
							...
							for(int i = 0; i < list.size(); i++) {
								String send = "OffLine//"+list.get(i)[0]+"//"+list.get(i)[1]+"//"+list.get(i)[2];
				...
				if(whatToDo.equals("Offline")) {
					...
					if(users.get(receiver)[2].equals("on")) {
						...
						// receiver does not respond, update table, broadcast and save message.
							...
							saveMessage(new String[]{receiver,time,sender,message},senderIP,senderPort);
						}
					}
					// server finds the receiver offline
					else saveMessage(new String[]{receiver,time,sender,message},senderIP,senderPort);
				}
			}
			catch(IOException e) {System.err.println("IOException"+e);}
		}
